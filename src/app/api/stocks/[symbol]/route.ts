...
function historicalMinuteUrl(
  ...
  return `https://api.upstox.com/v3/historical-candle/${encodeURIComponent(
    instrumentKey
  )}/minutes/5/${targetDate}/${subtractDays(targetDate, lookbackDays)}`
}
...
async function fetchOneDayCandles({
  ...
  if (isLiveToday) {
    const intradayUrl = `https://api.upstox.com/v2/historical-candle/intraday/${encodeURIComponent(
      instrumentKey
    )}/5minute`
    const intradayCandles = await fetchCandles(intradayUrl, accessToken)
...
export async function GET(
  ...
  try {
    ...
    const cacheDateKey = range === "1D" ? dateParam || todayStr : dateParam || "latest"
    ...
    if (range === "1D") {
      ...
      if (range === "1W") {
        ...
        url = `https://api.upstox.com/v3/historical-candle/${encodeURIComponent(
          instrumentKey
        )}/minutes/15/${todayStr}/${getIstDateString(fromDate)}`
      } else if (range === "1M") {
        fromDate.setMonth(today.getMonth() - 1)
        url = `https://api.upstox.com/v3/historical-candle/${encodeURIComponent(
          instrumentKey
        )}/minutes/60/${todayStr}/${getIstDateString(fromDate)}`
      } else {
        fromDate.setFullYear(today.getFullYear() - 1)
        url = `https://api.upstox.com/v2/historical-candle/${encodeURIComponent(
          instrumentKey
        )}/day/${todayStr}/${getIstDateString(fromDate)}`
      }

      candles = await fetchCandles(url, accessToken)
    }

    const result = toChartData(candles)

    if (result.length > 0) {
      const ttl = range === "1D" && marketOpen ? 60 : 300
